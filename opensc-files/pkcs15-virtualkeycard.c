...
//Had to be renamed, because sc_pkcs15init_get_muscle_ops did not work due to includes
struct sc_pkcs15init_operations * sc_pkcs15init_get_virtual_keycard_ops(void)
{
	return &sc_pkcs15init_muscle_operations;
}

void * sc_module_init(const char * string){
	printf("========== Advice for Virtual Keycard users ==========\nIf you are creating the PKCS15 structure (i.e., you executed "
	"pkcs15-init -C or pkcs15-init --create-pkcs15), please note that the PIN you will enter will not be set in Virtual Keycard "
	"on your android smartphone. You should already have set the PIN when you started the app. This is, because we do not want to "
	"send the PIN unencrypted to the smartphone. Just type \"123456\" or anything you want.\n====== End of advice for Virtual "
	"Keycard users =======\n");
	return &sc_pkcs15init_get_virtual_keycard_ops;
}
